...
getHistory <- function(symbol, type, startDate = NULL, endDate = NULL, maxRecords = '1000000',
   ...
   getMethod <- paste0(getOption("url"),
                       ...
                       '&nearby=', nearby,
                       '&jerq=', jerq,
                       '&exchange=', exchange, # 'NYSE%2CAMEX%2CNASDAQ',
   ...
   if (nrow(res_df) > 0 && output_type == 'xts') {
      res_df <- xts::as.xts(res_df[, c(-(1:3))],
                            order.by = as.POSIXct(res_df[, 2], format = "%Y-%m-%dT%H:%M:%S"))
